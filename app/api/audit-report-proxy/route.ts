...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const fetchHeaders: Record<string, string> = {
      Accept: "*/*",
      "User-Agent": "Mozilla/5.0 (compatible; AuditReportProxy/1.0)",
    };
    ...
    // Check content type - if it's not HTML, return as binary
    const contentType = response.headers.get("content-type") || "";
    const isHtml = contentType.includes("text/html") || contentType.includes("application/xhtml");
    
    if (!isHtml) {
      // For binary files (Excel, ZIP, etc.), return as-is
    ...
    for (const imgSrc of imageUrls) {
      try {
        ...
        const imgResponse = await fetch(absUrl, {
          headers: { "User-Agent": "Mozilla/5.0 (compatible; AuditReportProxy/1.0)" },
        });
        if (imgResponse.ok) {
    ...
    console.error("Audit report proxy error:", error);
    return NextResponse.json(
      { error: "Failed to fetch audit report HTML" },
      { status: 500 }
    );
  }
}
